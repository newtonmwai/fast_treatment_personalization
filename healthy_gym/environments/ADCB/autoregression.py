...
def fit_auto_regressor(df, autoreg_key_lists, autoreg_steps, Y_col, class_or_reg, month=12, test_size=0.2, return_metrics=True, grid_search=config.grid_search):
    ...
    if(class_or_reg == 'CLASSIFICATION'):
        ...
        if grid_search:
            ...
            with open(config.data_path + 'models/' + Y_col + '_autoreg_model.pkl', 'wb') as f:
                pickle.dump(lr, f)
            if not return_metrics:
                return lr
            else:
...
def generate_N_Samples_bl_n_dim(num_repetition, N, P1, PTETHCAT_probs, PTRACCAT_probs, PTGENDER_probs, AB_ratio_bl_df, gamma_0, gamma_1, epsilon=0.1, return_metrics=config.return_metrics):
    np.random.seed(num_repetition)
    ...
    if return_metrics:
        ...
        resRegressors = pd.DataFrame(
            [resTAU, resPTAU, resFDG, resAV45, resADAS13, resCDRSB, resMMSE])
        resClassifiers.to_csv(config.data_path + 'data/resClassifiers_bl.csv',
                              encoding='utf-8', index=False)
        resRegressors.to_csv(config.data_path + 'data/resRegressors_bl.csv',
                             encoding='utf-8', index=False)
        baseline_results = [resClassifiers, resRegressors]
    ...
    for i in range(N):
        ...
        while TAU < 0:
            TAU = lrTAU.predict(dm.check_categorical(pd.DataFrame(ddict), config.all_pred_cols['TAU'], 'TAU'))[
                0] + sample_normal_noise(AB_ratio_bl_df, 'TAU', 1)[0]  # sample_asymetric(residuals['TAU'].values, 1)[0]

        PTAU = lrPTAU.predict(dm.check_categorical(pd.DataFrame(
            ddict), config.all_pred_cols['PTAU'], 'PTAU'))[0] + noises['PTAU'][i]
        while PTAU < 0:
            PTAU = lrPTAU.predict(dm.check_categorical(pd.DataFrame(ddict), config.all_pred_cols['PTAU'], 'PTAU'))[
                0] + sample_normal_noise(AB_ratio_bl_df, 'PTAU', 1)[0]  # sample_asymetric(residuals['PTAU'].values, 1)[0]
        ...
        while FDG < 0:
            FDG = lrFDG.predict(dm.check_categorical(pd.DataFrame(ddict), config.all_pred_cols['FDG'], 'FDG'))[
                0] + sample_normal_noise(AB_ratio_bl_df, 'FDG', 1)[0]  # sample_asymetric(residuals['FDG'].values, 1)[0]  #

        AV45 = lrAV45.predict(dm.check_categorical(pd.DataFrame(
            ddict), config.all_pred_cols['AV45'], 'AV45'))[0] + noises['AV45'][i]
        while AV45 < 0:
            AV45 = lrAV45.predict(dm.check_categorical(pd.DataFrame(ddict), config.all_pred_cols['AV45'], 'AV45'))[
                0] + sample_normal_noise(AB_ratio_bl_df, 'AV45', 1)[0]  # sample_asymetric(residuals['AV45'].values, 1)[0]  #

        ddict = {'PTETHCAT': PTETHCAT, 'PTRACCAT': PTRACCAT,  'PTEDUCAT': PTEDUCAT, 'PTGENDER': PTGENDER, 'PTMARRY': PTMARRY,
                 'Z': Z, 'TAU': TAU, 'PTAU': PTAU, 'FDG': FDG, 'AV45': AV45}

        ADAS13 = lrADAS13.predict(dm.check_categorical(pd.DataFrame(
            ddict), config.all_pred_cols['ADAS13'], 'ADAS13'))[0] + noises['ADAS13'][i]  # np.random.normal(0, config.ADAS_NOISE / 2, size=1)  # +
        while ADAS13 < 0:
            ADAS13 = lrADAS13.predict(dm.check_categorical(pd.DataFrame(ddict), config.all_pred_cols['ADAS13'], 'ADAS13'))[
                0] + sample_normal_noise(AB_ratio_bl_df, 'ADAS13', 1)[0]  # sample_noise(AB_ratio_bl_df, 'ADAS13', 1)[0]
        if ADAS13 > 85:
            ADAS13 = 85

        ddict = {'PTETHCAT': PTETHCAT, 'PTRACCAT': PTRACCAT,  'PTEDUCAT': PTEDUCAT, 'PTGENDER': PTGENDER, 'PTMARRY': PTMARRY,
                 'Z': Z, 'TAU': TAU, 'PTAU': PTAU,  'FDG': FDG, 'AV45': AV45, 'ADAS13': ADAS13}

        MMSE = lrMMSE.predict(dm.check_categorical(pd.DataFrame(
            ddict), config.all_pred_cols['MMSE'], 'MMSE'))[0] + noises['MMSE'][i]
        while MMSE < 0:
            MMSE = lrADAS13.predict(dm.check_categorical(pd.DataFrame(ddict), config.all_pred_cols['MMSE'], 'MMSE'))[
                0] + sample_normal_noise(AB_ratio_bl_df, 'MMSE', 1)[0]  # sample_noise(AB_ratio_bl_df, 'MMSE', 1)[0]
        if MMSE > 30:
            MMSE = 30
        ...
        while CDRSB < 0:
            CDRSB = lrCDRSB.predict(dm.check_categorical(pd.DataFrame(ddict), config.all_pred_cols['CDRSB'], 'CDRSB'))[
                0] + sample_normal_noise(AB_ratio_bl_df, 'CDRSB', 1)[0]  # sample_noise(AB_ratio_bl_df, 'CDRSB', 1)[0]
        if CDRSB > 18:
            CDRSB = 18
        ...
        ddict = {'PTETHCAT': PTETHCAT, 'PTRACCAT': PTRACCAT, 'PTGENDER': PTGENDER, 'Z': Z,
                 'TAU': TAU, 'PTAU': PTAU, 'FDG': FDG, 'AV45': AV45, 'ADAS13': ADAS13}
        # DX = lrDX.predict(pd.DataFrame(ddict))[0]
        DX = lrDX.predict_proba(dm.check_categorical(
            pd.DataFrame(ddict), config.all_pred_cols['DX'], 'DX'))
        DX = choice(len(DX[0]), 1, p=list(DX[0]))[0]
        ...
        ddict = {'RID': i, 'AGE': AGE, 'VISCODE': 0, 'PTETHCAT': PTETHCAT[0], 'PTRACCAT': PTRACCAT[0],
                 'PTMARRY': PTMARRY, 'PTEDUCAT': PTEDUCAT, 'PTGENDER': PTGENDER[0], 'Z': Z,
                 'TAU': TAU, 'PTAU': PTAU, 'FDG': FDG, 'AV45': AV45, 'ADAS13': ADAS13, 'MMSE': MMSE,
                 'CDRSB': CDRSB, 'DX': DX, 'A': A, 'Delta': delta, 'Y_hat': Y_hat,
                 'Y_0': Y_0, 'Y_1': Y_1, 'Y_2': Y_2, 'Y_3': Y_3, 'Y_4': Y_4, 'Y_5': Y_5, 'Y_6': Y_6, 'Y_7': Y_7}
...
def gen_autoregression_samples_n_dim(N, num_repetition, AB_ratio_bl_df, gen_data, df, autoreg_key_lists,
                                     autoreg_steps, num_steps, gamma_0, gamma_1, P1, epsilon=0.1, policy='DX', return_metrics=config.return_metrics):
    np.random.seed(num_repetition)
    last_month = num_steps * 12  # + 12

    df2 = pd.DataFrame()
    df2 = df2.append(gen_data[['RID', 'AGE', 'PTETHCAT', 'PTRACCAT', 'PTGENDER', 'PTEDUCAT', 'PTMARRY', 'TAU', 'PTAU', 'FDG', 'AV45', 'Z',
                     'VISCODE', 'ADAS13', 'MMSE', 'CDRSB', 'DX', 'A', 'Delta', 'Y_hat', 'Y_0', 'Y_1', 'Y_2', 'Y_3', 'Y_4', 'Y_5', 'Y_6', 'Y_7']], ignore_index=True)
    ...
    if return_metrics:
        ...
        resRegressors = pd.DataFrame(
            [resTAU, resPTAU, resFDG, resAV45, resADAS13, resMMSE, resCDRSB])
        resClassifiers.to_csv(config.data_path + 'data/resClassifiers_auto_' + str(policy) + '.csv',
                              encoding='utf-8', index=False)
        resRegressors.to_csv(config.data_path + 'data/resRegressors_auto_' + str(policy) + '.csv',
                             encoding='utf-8', index=False)
        autoreg_results = [resClassifiers, resRegressors]
    ...
    for month in range(12, last_month, 12):
        ...
        # + np.random.normal(0, ADAS13RMSE/2, size=(len(df2.loc[df2['VISCODE'] == (month-12)]))).reshape(-1) #+ noises['ADAS13']
        ADAS13 = lrADAS13_auto.predict(dm.check_categorical(
            df_past, list(df_past.columns), '_')) + noises['ADAS13']  # + np.random.normal(0, config.ADAS_NOISE / 2, size=(len(df2.loc[df2['VISCODE'] == (month - 12)]))).reshape(-1)  #
        ...
        ddict = {'RID': RID, 'AGE': AGE, 'PTETHCAT': PTETHCAT, 'PTRACCAT': PTRACCAT, 'PTGENDER': PTGENDER, 'PTEDUCAT': PTEDUCAT,
                 'PTMARRY': PTMARRY, 'TAU': TAU, 'PTAU': PTAU, 'FDG': FDG, 'AV45': AV45, 'Z': Z,  'ADAS13': ADAS13, 'MMSE': MMSE, 'CDRSB': CDRSB}
        ...
        gen_data_auto['Delta'] = gen_data_auto.apply(
            lambda x: tr.assign_treatment_effect_n_dim(a=x['A'], Z=x['Z'], gamma=gamma_0), axis=1)
        ...
        dx_df = df2.loc[df2['VISCODE'] == (
            month - 12)][['DX', 'PTETHCAT', 'PTRACCAT',  'PTGENDER', 'Z', 'TAU', 'PTAU', 'FDG', 'AV45']]
        dx_df['ADAS13'] = gen_data_auto['Y_hat'].values
