...
class ModelBelief:
    ...
    def kl_matrix(self, x):
        ...
        means = np.array([model.predict(x).flatten() for model in self.models])
...
class GaussianModelBelief(ModelBelief):
    def __init__(self, models, prior=None, std=1, seed=None):
        """
    ...
    def sample_reward(self, x, a, n_draws=1):
        ...
        for i in range(n_draws):
            ...
            reward = self.random_state.normal(self.models[z].predict(x)[a], scale=self.std)
